...
def test_val_single_epoch(model, dataloader, device, topk, mask, user_his_iid, multicore=0):
    ...
    F1_res , Prec_res, Rec_res, NDCG_res = [], [], [], []
    for k in topk:
        X = zip(rating_list, groundTrue_list)
        Recall, Precision, NDCG = 0, 0, 0, 0
        for i, x in enumerate(X):
            precision, recall, ndcg = test_val_single_batch(x, k)
            Recall += recall
            Precision += precision
            NDCG += ndcg
        ...
        Rec_res.append(Recall)
        NDCG_res.append(NDCG)

    return F1_res , Prec_res, Rec_res, NDCG_res
...
def train(model, train_loader, val_loader, optimizer, stopper, device, mask, user_his_train, user_his_val):
    for epoch in tqdm.tqdm(range(args.epochs)):
        ...
        F1_score, Precision, Recall, NDCG = test_val_single_epoch(model, val_loader, device, args.topk, mask, user_his_val, args.test_multicore)

        print(f"\nTrain Epoch: {epoch + 1} Training Loss: {loss:.6f}")
        for i, k in enumerate(args.topk):
            print(f"\nF1_score: {F1_score[i]:6f} Precision: {Precision[i]:.6f} Recall: {Recall[i]:.6f} NDCG: {NDCG[i]:.6f}")

        # early stopping
        stopper(Precision[-1], model=model, args=args)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--topk', nargs='?', type=int, default=[20, 10], help="")
    ...
    # model
    model = AGIP(args, data_info, user_fea_index, item_fea_index, item_fea_mask, sp_graph, spk_emb_list).to(device)
    loss_fcn = CosineContrastiveLoss(margin=0.2)
    ...
    if args.test:
        best_model = torch.load(f'./checkpoints/{args.dataset}.pt')
        F1_score, Precision, Recall, NDCG = test_val_single_epoch(best_model, test_loader, device, args.topk, mask, user_his_test, args.test_multicore)
